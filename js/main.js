...
function onLogout() {
  ...
  firebase.database().ref("favorites").off("value");
...
function loadChatView() {
  ...
  // お気に入り一覧を取得してさらに変更を監視
  var favoritesRef = firebase.database().ref("favorites");
  // 過去に登録したイベントハンドラを削除
  favoritesRef.off("value");
  // イベントハンドラを登録
  favoritesRef.on("value", function(favoritesSnapshot) {
    // favoritesに変更があるとこの中が実行される
  ...
  roomsRef.on("value", function(roomsSnapshot) {
    ...
    // お気に入り一覧ルームが存在しない場合は作成する
    if (!dbdata.rooms[favoritesRoomName]) {
      console.log(favoritesRoomName + "ルームを作成します");
...
function createMessageDiv(messageId, message) {
  ...
  // お気に入りアイコンを表示
  if (dbdata.favorites === null || !dbdata.favorites[currentUID] || !dbdata.favorites[currentUID][messageId]) {
    // お気に入りが存在しない
    $div.find(".message__favorite").addClass("glyphicon-star-empty");
  } else {
    // お気に入りが存在する
    $div.find(".message__favorite").addClass("glyphicon-star on");
  }
  // お気に入りアイコンがクリックされた
  $div.find(".favorite__link").click(function() {
    // ハンバーガーメニューが開いている場合は閉じる
    $("#navbar").collapse("hide");
    
    var $this = $(this);
    
    // お気に入りリストが存在しない場合は作成する
    if (dbdata.favorites === null || !dbdata.favorites[currentUID] || !dbdata.favorites[currentUID][messageId]) {
      // お気に入りにする
      ...
      $this.find(".message__favorite").removeClass("glyphicon-star-empty").addClass("glyphicon-star on");
      console.log("お気に入りに追加しました");
    } else {
      // お気に入り済みなのでお気に入りを解除する
      firebase.database().ref("favorites/" + currentUID + "/" + messageId).remove();
      $this.find(".message__favorite").removeClass("glyphicon-star on").addClass("glyphicon-star-empty");
...
function _showRoom(roomName) {
  ...
  if (roomName === favoritesRoomName) {
    // お気に入り一覧を表示
    var favoriteRef = firebase.database().ref("favorites/" + currentUID).orderByChild("createdAt").limitToLast(5);
    favoriteRef.once("value").then(function(snapshot) {
      snapshot.forEach(function(childSnapshot) {
        addMessage(childSnapshot.key, childSnapshot.val().message);
      });
    });
  } else {
    // ルームのメッセージ一覧をダウンロードし、かつメッセージの追加を監視
    var roomRef = firebase.database().ref("messages/" + roomName).orderByChild("time").limitToLast(5);
    // 過去に登録したイベントハンドラを削除
  ...
  // 初期ルーム・お気に入り一覧ルームの場合はルーム削除メニューを無効にする
  ...
  // お気に入り一覧ルームの場合はメッセージ投稿フォームを無効にする
  if (roomName === favoritesRoomName) {
    $(".comment-form__text").prop("disabled", true);
    $(".comment-form__submit").prop("disabled", true);
  } else {
    $(".comment-form__text").prop("disabled", false);
...
function deleteRoom(roomName) {
  //  初期ルーム・お気に入り一覧ルームは削除不可
  if (roomName === defaultRoomName || roomName === favoritesRoomName) {
    throw new Error(roomName + "ルームは削除できません");
  }
...
$(document).ready(function() {
  ...
  $("#deleteRoomModal").on("show.bs.modal", function(event) {
    ...
    // 初期ルーム・お気に入り一覧ルームは削除不可のためモーダルを表示しない
    if (roomName === defaultRoomName || roomName === favoritesRoomName) {
      return false;
    }
